# CHI TIẾT TRIỂN KHAI HỆ THỐNG KAIRUST (KÈM SƠ ĐỒ VÀ CÁC LỆNH SETUP)

graph TD
    classDef myLocal fill:#1a1b26,stroke:#7aa2f7,stroke-width:2px,color:#cfc9c2
    classDef myServer fill:#1f2335,stroke:#f7768e,stroke-width:2px,color:#cfc9c2
    classDef myNetwork fill:#292e42,stroke:#bb9af7,stroke-width:2px,color:#cfc9c2
    
    subgraph Local_Computer["💻 MÁY CÁ NHÂN (Fedora)"]
        Code[Code Editor] -->|1. Lưu & Commit| GitRepo[(Kho Git Local)]
        SSHKey[Phát hành File Khóa: id_ed25519] -.->|3. Ủy quyền xác thực| SSHAgent(Trình SSH Agent Lõi)
        GitRepo -->|2. Cập nhật mã nguồn| GitHub[☁️ GitHub Repository]
    end

    subgraph Server_mysv["☁️ MÁY CHỦ INTERNET (Server mysv)"]
        SSHD[SSH Server] <-->|Liên kết nhận ủy quyền Agent Lõi| SSHAgent
        SSHD -->|4. git clone / pull bằng SSH| GitHub
        
        Docker[🐳 Môi trường Docker Compose]
        SSHD -->|5. Khởi động docker-compose up| Docker
        
        %% Các dịch vụ trong Docker
        Tunnel[kairust_tunnel: Cloudflare Tunnel]
        Caddy[kairust_caddy: Reverse Proxy 80, 443]
        Backend[kairust_backend: Node API / Port 3001]
        Frontend[kairust_frontend: Vite Web / Port 80]
    ...
    subgraph Internet_World["🌐 NGƯỜI DÙNG INTERNET"]
        UserBrowser[🖥️ Trình duyệt Web]
        Cloudflare[☁️ Mạng Cửa Ngõ Cloudflare]
    end

    %% Mạng lưới giao tiếp
    UserBrowser -->|Truy cập URL kairust.duckdns.org| Cloudflare
    Cloudflare <-->|Kết nối ngược an toàn| Tunnel
    Tunnel -->|Chuyển tiếp dữ liệu HTTP TCP nội bộ| Caddy
    
    Caddy -->|Phân mảnh: Nội dung giao diện Web| Frontend
    Caddy -->|Phân mảnh: Kết nối đường ngầm WebSocket /ws*| Backend
    
    Backend -->|Push Data & Compile| Compiler[🛠️ Rust 1.85 Engine]
    
    class Local_Computer,Code,GitRepo,SSHKey,SSHD,SSHAgent myLocal;
    class Server_mysv,Docker,Tunnel,Caddy,Backend,Frontend,Compiler myServer;
    class Internet_World,UserBrowser,Cloudflare,GitHub myNetwork;
